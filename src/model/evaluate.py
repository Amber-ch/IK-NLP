...
def clean_df(df, args):
    ...
    for i in range(1, 4):
        for metric in eval:
            drop_cols.append(
                f"rug-nlp-nli/flan-base-nli-explanation{model_suffix}_{metric}_{i}"
            )
            drop_cols.append(
                f"rug-nlp-nli/flan-base-nli-label-explanation{model_suffix}_{metric}_{i}"
            )

    # Remove list of explanations column
    drop_cols.append(
        f"rug-nlp-nli/flan-base-nli-label-explanation{model_suffix}_labels")
    drop_cols.append(
        f"rug-nlp-nli/flan-base-nli-explanation{model_suffix}_labels")

    # Remove duplicate prediction column
    drop_cols.append(
        f"rug-nlp-nli/flan-base-nli-explanation{model_suffix}_prediction.1")
    drop_cols.append(
        f"rug-nlp-nli/flan-base-nli-label-explanation{model_suffix}_prediction.1"
    )
...
def evaluate_model(model_name, task, input, target_explanations, target_labels,
    ...
    generated_predictions = generate_predictions(model, tokenizer, input,
                                                 device)
    # Split predictions in labels and explanations, depending on what the model produced
    generated_labels, generated_explanations = split_predictions(
        task, generated_predictions, args)

    # create empty dataframe
    results = pd.DataFrame()
    # First evaluate explanations:
    if generated_explanations is not None:
        if eval_type in ['neural', 'both']:
            # Calculate scores
            neural_results = neural_evaluation_explanations(
                model_name, generated_predictions, target_explanations, device)
        ...
        if eval_type in ['text', 'both']:
            ...
            text_results = text_evaluation_explanations(
                model_name, generated_predictions,
                target_explanations)  # for later, when we have AMBER
    ...
    if generated_labels is not None:
        # Evaluate labels
        label_results = evaluate_labels(generated_labels, target_labels)
...
def generate_predictions(model, tokenizer, input, device):
    ...
    for i in tqdm(range(len(input))):
        input[i] = tokenizer(input[i], truncation=True,
                             return_tensors="pt").to(device)
        output = model.generate(**input[i],
                                num_beams=8,
                                do_sample=True,
                                max_new_tokens=64)
...
def neural_evaluation_explanations(model_name, predictions, target, device):
    ...
    # Calculate scores
    results = bart_scorer.multi_ref_score(predictions,
                                          target,
                                          agg="max",
                                          batch_size=4)
    ...
    results.to_csv(
        'results/{}_neural_scores.csv'.format(model_name.partition('/')[2])
        )  # save to csv file.
    return results
...
def text_evaluation_explanations(model_name, predictions, target):
    ...
    rouge_scores_explode[['label_1', 'label_2', 'label_3']] = pd.DataFrame(
        scores_labels_predictions_df.labels.tolist(),
        index=scores_labels_predictions_df.index)
    rouge_scores_explode[rouge_1_columns] = pd.DataFrame(
        scores_labels_predictions_df.rouge_1.tolist(),
        index=scores_labels_predictions_df.index)
    rouge_scores_explode[rouge_2_columns] = pd.DataFrame(
        scores_labels_predictions_df.rouge_2.tolist(),
        index=scores_labels_predictions_df.index)
    rouge_scores_explode[rouge_L_columns] = pd.DataFrame(
        scores_labels_predictions_df.rouge_L.tolist(),
        index=scores_labels_predictions_df.index)
    ...
    rouge_scores_explode.to_csv('results/{}_rouge_scores.csv'.format(model_name.partition('/')[2]))
...
def generate_summary(results, args):
    ...
    # Assign models based on locally stored files or the huggingface repository
    if args.custom_label_model and args.custom_explanation_model and args.custom_label_explanation_model:
        label_model_name = f'{MODEL_DIR}/{args.custom_label_model}'
        explanation_model_name = f'{MODEL_DIR}/{args.custom_explanation_model}'
        label_explanation_model_name = f'{MODEL_DIR}/{args.custom_label_explanation_model}'
    else:
        label_model_name = f'rug-nlp-nli/flan-base-nli-label{model_suffix}'
        explanation_model_name = f'rug-nlp-nli/flan-base-nli-explanation{model_suffix}'
        label_explanation_model_name = f'rug-nlp-nli/flan-base-nli-label-explanation{model_suffix}'
    ...
    if eval_type in ['neural', 'both']:
        ...
        mean = round(
            results[
                f'{explanation_model_name}_neural_score']
            .mean(), 2)
        std = round(
            results[
                f'{explanation_model_name}_neural_score']
        ...
        # Find average and standard deviation of column rug-nlp-nli/flan-base-nli-label-explanation_neural_score
        mean = round(
            results[
                f'{label_explanation_model_name}_neural_score']
            .mean(), 2)
        std = round(
            results[
                f'{label_explanation_model_name}_neural_score']
    ...
    if eval_type in ['text', 'both']:
        ...
        mean = round(
            results[
                f'{label_explanation_model_name}_rouge_1_max']
            .mean(), 2)
        std = round(
            results[
                f'{label_explanation_model_name}_rouge_1_max']
            .std(), 2)
        summary += f"Average explanation text score of {explanation_model_name}_rouge_1_max: " + \
            str(mean) + " (std: " + str(std) + ")\n"

        # Find average and standard deviation of column rug-nlp-nli/flan-base-nli-explanation_rouge_2_max
        mean = round(
            results[
                f'{explanation_model_name}_rouge_2_max']
            .mean(), 2)
        std = round(
            results[
                f'{explanation_model_name}_rouge_2_max']
            .std(), 2)
        summary += f"Average explanation text score of {explanation_model_name}_rouge_2_max: " + \
            str(mean) + " (std: " + str(std) + ")\n"

        # Find average and standard deviation of column rug-nlp-nli/flan-base-nli-explanation_rouge_L_max
        mean = round(
            results[
                f'{explanation_model_name}_rouge_L_max']
            .mean(), 2)
        std = round(
            results[
                f'{explanation_model_name}_rouge_L_max']
            .std(), 2)
        summary += f"Average explanation text score of {explanation_model_name}_rouge_L_max: " + \
            str(mean) + " (std: " + str(std) + ")\n"
        ...
        std = round(
            results[
                f'{label_explanation_model_name}_rouge_1_max']
            .std(), 2)
        summary += f"Average explanation text score of {label_explanation_model_name}_rouge_1_max: " + \
            str(mean) + " (std: " + str(std) + ")\n"

        # Find average and standard deviation of column rug-nlp-nli/flan-base-nli-explanation_rouge_2_max
        mean = round(
            results[
                f'{label_explanation_model_name}_rouge_2_max']
            .mean(), 2)
        std = round(
            results[
                f'{label_explanation_model_name}_rouge_2_max']
            .std(), 2)
        summary += f"Average explanation text score of {label_explanation_model_name}_rouge_2_max: " + \
            str(mean) + " (std: " + str(std) + ")\n"

        # Find average and standard deviation of column rug-nlp-nli/flan-base-nli-explanation_rouge_L_max
        mean = round(
            results[
                f'{label_explanation_model_name}_rouge_L_max']
            .mean(), 2)
        std = round(
            results[
                f'{label_explanation_model_name}_rouge_L_max']
            .std(), 2)
        summary += f"Average explanation text score of {label_explanation_model_name}_rouge_L_max: " + \
            str(mean) + " (std: " + str(std) + ")\n"

    # Find percentage of correct labels of nli-label
    correct_labels = results[
        f'{label_model_name}_correct_label?'].sum()
    total_labels = len(results[
        f'{label_model_name}_correct_label?'])
    percentage = str(round(correct_labels / total_labels, 2))
    summary += f"Percentage of correct labels of {label_model_name}: " + percentage + "\n"

    # Find percentage of correct labels of nli-label-explanation
    correct_labels = results[
        f'{label_explanation_model_name}_correct_label?'].sum(
        )
    total_labels = len(results[
        f'{label_explanation_model_name}_correct_label?']
                       )
    percentage = str(round(correct_labels / total_labels, 2))
    summary += f"Percentage of correct labels of {label_explanation_model_name}: " + percentage + "\n"
...
def compute_metrics(predictions, targets):
    ...
    for i in range(len(predictions)):
        result = [
            metric.compute(predictions=[predictions[i]] * 3,
...
def run(args):
    ...
    full_test_set = pd.DataFrame(dataset["test"],
                                 columns=[
    ...
    full_test_set = full_test_set.head(
        int(len(full_test_set) * args.subset_size))
    ...
    input_nli_label_explanation = input_nli_label_explanation.tolist(
    )  # Convert to list
    ...
    test_target_explanations = test_target_explanations.values.tolist(
    )  # Convert to list
    ...
    # Evaluate models
    print(
        f"Evaluating model: {explanation_model_name}"
    )
    model_results_explanation = evaluate_model(
        explanation_model_name,
        1,
        input_nli_explanation, test_target_explanations, test_target_labels,
        args)

    print(f"Evaluating model: {label_model_name}")
    model_results_label = evaluate_model(
        label_model_name,
        0,
        input_nli_label,
        test_target_explanations, test_target_labels,
        args)

    print(
        f"Evaluating model: {label_explanation_model_name}"
    )
    model_results_label_explanation = evaluate_model(
        label_explanation_model_name,
        2,
        input_nli_label_explanation, test_target_explanations,
        test_target_labels, args)

    print("Exporting results to .csv file...")
    # Make dataframe with the input and the results
    results = pd.concat([
        full_test_set, model_results_explanation, model_results_label,
        model_results_label_explanation
    ],
                        axis=1)
    ...
    if eval_type in ['neural', 'both']:
        results = results.sort_values(
            by=
            '{}_neural_score'.format(label_explanation_model_name),
            ascending=True)
